@testset "test_pointwise_ops.jl" begin
    ...
    @testset "binary pointwise" begin
        pw_a = Piecewise([-1], [0,1])
        pw_b = Piecewise([1], [0,1])
    ...
    @testset "repeated boundaries" begin
        pw_a = Piecewise([0], [-1,1])
        pw_b = Piecewise([0], [1,-1])
        pw_c = Piecewise([0], [0,1])
    ...
    @testset "interleaved boundaries" begin
        pw_a = Piecewise([0,2], ["lo", "hi", "lo"])
        pw_b = Piecewise([1], ["lo", "hi"])
